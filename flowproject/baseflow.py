...
class BaseFlow(FlowSpec):
        ...
        if self.flowconfig.data_kwargs.check_mode == 'files_metadata':
            ...
            with S3(
                ...
                if self.flowconfig.data_kwargs.key and (
                    self.flowconfig.data_kwargs.key == '' or \
            ...
            raise ValueError(f'{self.flowconfig.data_kwargs.check_mode} is not supported. Use one of files_metadata, file_modified_ts, or file_size')
    ...
    def query(self, storage_type, card=False, **kwargs):
        if storage_type == 'snowflake':
            ...
            # TODO: Pick default smarter. Validate SQL file exists somewhere.
            return self._query_snowflake(template=template, card=card)
        elif storage_type == 's3':
            return self._query_s3(card=card)
        else:
            raise ValueError(f"Invalid storage_type in self.query {storage_type}. Supported options include ['snowflake','s3'].")
